...
public class FMLEventHandler {
    ...
    public void onChat(ClientChatEvent event) {
        if (event.getOriginalMessage().startsWith(TTC.prefix)) {
            ...
            ChatUtils.print("§a[ATC] §rThis message was not sent to the server");
            ...
            try {
                ...
                if (s.equals("help")) {
                    ...
                    if (help == null) {
                        ChatUtils.print("§a[ATC] §cUnable retrieve help message!");
                    } else {
            ...
            while (TTC.mc.world != null) {
                if(TTC.globalConfig.getSub("messages").getBoolean("update")) {
                    if(Update.send) {
                        ...
                        if (s == null) {
                            ChatUtils.print("§a[ATC] §cUnable to check for a new version");
                        }
                        else if (!s.equals(TTC.VERSION)) {
                            ChatUtils.print(
                                    "§a§lA new ATC version was found! Current: " +
                                    TTC.VERSION +
                                    ", New: " +
                                    s +
                                    "You can update at " +
                                    "https://github.com/vibrateur/ATClient/releases/tag/" +
                                    s +
                                    " or type ',update'"
                            );
                        }
    ...
    public void onDeath(EntityPlayer player) {
        ...
        ChatUtils.print("You died at " + pos.getX() + " " + pos.getY() + " " + pos.getZ());
    }
