        ...
        try {
            ...
            switch (packet.type()) {
                ...
                case NAME:
                    ...
                    ChatUtils.print("§a[ATC] §rConnection to main " + main.name + " established!");
                    sendPacket(PacketsCS.UUID, TTC.mc.getSession().getProfile().getId().toString());
                    break;
                case UUID:
                    main.uuid = UUID.fromString(packet.content());
                    ChatUtils.print("§a[ATC] §rGot UUID from main " + main.name + ": " + packet.content());
                    sendPacket(PacketsCS.KEEPALIVE, "");
                    break;
                case TPA:
                    ChatUtils.print("§a[ATC] §rTPA'ing main account...");
                    TTC.player.sendChatMessage("/tpa " + main.name);
                    break;
                case EXECUTE:
                    ChatUtils.print("§a[ATC] §rSending message received from main account...");
                    ChatUtils.simulateSend(packet.content(), false);
                    break;
                case LIST:
                    TTC.logger.info("§a[ATC] §rReceived alt list from main.");
                    ...
                    break;
                case KILL:
                    ChatUtils.print("§a[ATC] §rKilling player " + packet.content());
                    kill(packet.content());
                    break;
                case FOLLOW:
                    ChatUtils.print("§a[ATC] §rFollowing " + packet.content());
    ...
    public void onPacketCS(PacketCS packet, PBIC.Connection connection) throws PBIC.PBICException.PBICWriteException {
        ...
        switch (packet.type()) {
            case NAME:
                ...
                ChatUtils.print("§a[ATC] §rConnection to alt " + packet.content() + " established!");
            ...
            case UUID:
                ...
                ChatUtils.print("§a[ATC] §rGot UUID from alt " + altsMap.get(connection).name + ": " + packet.content());
    ...
    public void onChat(String s, String[] args) {
        try {
            if (s.equals("server") && server == null) {
                ...
                ChatUtils.print("§a[ATC] §aServer started");
            }
            if (args[0].equals("client") && client == null) {
                ...
                ChatUtils.print("§a[ATC] §aClient started");
            ...
            if(args.length >= 2) {
                ...
                if(args[0].equals("stop") && s.contains(" ")) {
                    ...
                    ChatUtils.print("§a[ATC] §rStopping killing player " + st);
                    if(botMain) {
                        stop(st);
                    }
                }
            ...
            if (s.equals("stop")) {
                ...
                ChatUtils.print("§a[ATC] §rStopping killing/following all players");
                if(botMain) {
                    stop(null);
                }
            }
            ...
            if (s.equals("send")) {
                ...
                ChatUtils.print("§a[ATC] §rSending config to all alts");
            }
    ...
    public void stop(String name) {
        ...
        if(name == null || name.equals("")) {
            ...
            if (aura.targets.size() != 0) {
                ChatUtils.print("§a[ATC] §rKilling player " + name);
