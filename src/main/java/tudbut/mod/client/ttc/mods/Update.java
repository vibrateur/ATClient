        ...
        try {
            ...
            new Thread(() -> {
                try {
                    ...
                    File old = new File("mods/old/atc/" + TTC.VERSION);
                    old.mkdirs();
                    for (File file : mods.listFiles()) {
                        if(file.getName().startsWith("atc") && file.getName().endsWith(".jar")) {
                            if(!file.renameTo(new File(old, file.getName()))) {
                                file.delete();
                            }
                        }
                    }
                    File f = new File("mods/atc.jar");
                    FileOutputStream stream = new FileOutputStream(f);
                    stream.write(bytes);
                    ChatUtils.print("Finishing off...");
                    stream.close();
                    ChatUtils.print("Done! Next time you launch minecraft, your AT will be on version " + version);
                    send = false;
                }
                catch (Exception e) {
                    ChatUtils.print("Couldn't update!");
